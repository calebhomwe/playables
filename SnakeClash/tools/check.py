"""Static gate on the single-file game (SNAKE CLASH).

Hard constraint: OFFLINE — zero network. Any external reference is a FAIL.
Also parses the inline <script> with node --check, and sanity-checks the arena
snake constants, the persistence keys and the debug hook wiring.
...
# --- game contract ------------------------------------------------------
for need in ("window.__snake_debug", "requestAnimationFrame", "getContext",
             "visibilitychange", "localStorage", "sc_rec", "sc_snd",
             'id="play"', 'id="again"', 'id="resume"', 'id="s-over"',
             'id="s-menu"', 'id="s-pause"', 'id="hud"', 'id="len"',
             'id="score"', 'id="rank"', 'id="best"', 'id="sndBtn"',
             "ARENA", "WALL", "SPACING", "MAXSEG", "SEG_R", "HEAD_R",
             "START_LEN", "MIN_BOOST_LEN", "ORB_R", "ORB_GROWTH", "ORB_TARGET",
             "BASE_SPEED", "BOOST_MULT", "BURN_RATE", "TURN_RATE", "KEY_TURN",
             "RIVAL_SPEED", "RIVAL_START", "RIVAL_MAX", "RIVAL_RAMP",
             "spawnOrbAt", "setLength", "setHead", "forceRival", "kill",
             "clearArena", "setSpawner", "setHeading", "setBoost",
             "freeze(", "advance(", "rankOf", "burst("):
    if need not in src:
