"""Static gate on the single-file game (HELIX DROP).

Hard constraint: OFFLINE — zero network. Any external reference is a FAIL.
Also parses the inline <script> with node --check, and sanity-checks the helix
constants, the persistence keys and the debug hook wiring.
...
# --- game contract ------------------------------------------------------
for need in ("window.__helix_debug", "requestAnimationFrame", "getContext",
             "visibilitychange", "localStorage", "hx_rec", "hx_snd",
             'id="play"', 'id="again"', 'id="resume"', 'id="s-over"',
             'id="s-menu"', 'id="s-pause"', 'id="hud"', 'id="depth"',
             'id="best"', 'id="sndBtn"',
             "SHAFT_W", "SEG_COUNT", "GAP_CELLS", "LEVEL_H", "PLAT_H",
             "BALL_R", "GRAVITY", "BOUNCE_V", "ROT_SPEED", "ROT_RAMP",
             "ROT_MAX", "MAX_FALL", "DANGER_MAX", "DEPTH_PER_LEVEL",
             "SECTOR_NAMES",
             "setBallX", "rotateTo", "forceDanger", "alignGap", "setDepth",
             "clearTower", "freeze(", "advance("):
    if need not in src:
